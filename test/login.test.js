...
describe('LOGIN', () => {
  ...
  it('username should be', (done) => {
    chai
      ...
      .end((error, res) => {
        expect(res.text).equals('Wrong username. Should be longer than 2 and shorter then 128');
  ...
  it('password should be', (done) => {
    chai
      ...
      .end((error, res) => {
        expect(res.text).equals('Wrong password. Should be longer than 6 and shorter then 128');
        expect(res.statusCode).to.equal(400);
        done();
      });
  });
