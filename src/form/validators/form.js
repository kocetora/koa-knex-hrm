...
const form = (ctx, next) => {
  ...
  if (!phoneNumber || !isLength(phoneNumber, {
    min: 5,
    max: 20
  })) {
    return ctx.throw(400,
  ...
  if (!born || !isDate(born)) {
    return ctx.throw(400,
      'Incorrect born date. Should be YYYY-MM-DD');
  }
  ...
  if (!sex || !isIn(sex, [
    ...
    return ctx.throw(400,
      'Incorrect sex. May be male or female only');
  }
  if (note !== undefined && !isLength(note, {
    min: 1,
    max: 255
  })) {
    return ctx.throw(400,
  ...
  if (!height || !isInt(height.toString(), {
    min: 30,
    max: 300
  })) {
    return ctx.throw(400,
  ...
  if (!expectedSalary || !isInt(expectedSalary.toString(), {
    min: 1,
    max: 2147483647
  })) {
    return ctx.throw(400,
  ...
  languageSkills.forEach(el => {
    ...
    if (!el.languageProficiency || !isIn(el.languageProficiency, [
      'native',
      'fluent',
      'intermediate',
      'basic'
  ...
  professions.forEach(el => {
    if (!el.profession || !isIn(el.profession, [
      'trainee',
      'dealer',
      'inspector',
      'manager',
  ...
  messengers.forEach(el => {
    if (!el.messenger || !isIn(el.messenger, [
      'Telegram',
      'Viber',
      'WhatsApp'
    ])) {
